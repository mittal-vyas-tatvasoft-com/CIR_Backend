...
namespace CIR.Application.Services.GlobalConfiguration
{
    public class GlobalConfigurationWeekendsService : IGlobalConfigurationWeekendsService
    {
        ...
        public async Task<IActionResult> GetGlobalConfigurationWeekends(int displayLength, int displayStart, string? sortCol, string? filterCountryCode, string? filterCountryName, string? search, bool sortAscending = true)
        {
            return await _globalConfigurationWeekendsRepository.GetGlobalConfigurationWeekends(displayLength, displayStart, sortCol, filterCountryCode, filterCountryName, search, sortAscending);
        }
    }
}
