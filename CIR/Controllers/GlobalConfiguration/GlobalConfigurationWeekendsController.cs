        ...
        public async Task<IActionResult> Create([FromBody] GlobalConfigurationWeekends weekend)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (weekendExists)
                    {
                        return new JsonResult(new CustomResponse<string>() { StatusCode = (int)HttpStatusCodesAndMessages.HttpStatus.BadRequest, Result = false, Message = HttpStatusCodesAndMessages.HttpStatus.BadRequest.GetDescriptionAttribute(), Data = string.Format(SystemMessages.msgDataExists, "Weekend") });
                    }
                    else
                    {
