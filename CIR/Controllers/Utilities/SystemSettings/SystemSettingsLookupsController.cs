...
namespace CIR.Controllers.Utilities.SystemSettings
{
	...
	public class SystemSettingsLookupsController : ControllerBase
	{
		...
		[HttpGet("[action]")]
		public async Task<IActionResult> Get(string sortCol, string? searchCultureCode, string? searchLookupItems, bool sortAscending = true)
		{
			try
			{
				...
				var lookupModel = await _lookupService.GetAllLookupsItems(sortCol, searchCultureCode, searchLookupItems, sortAscending);

				if (lookupModel != null)
				{
					return new JsonResult(new CustomResponse<LookupsModel>() { StatusCode = (int)HttpStatusCodes.Success, Result = true, Message = HttpStatusCodesMessages.Success, Data = (LookupsModel)lookupModel });
				}
