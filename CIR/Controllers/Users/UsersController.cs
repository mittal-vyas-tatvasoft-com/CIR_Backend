	...
	public class UsersController : ControllerBase
	{
		...
		public async Task<IActionResult> Create([FromBody] User user)
		{
			if (ModelState.IsValid)
			{
				try
				{
					...
					if (userExists)
					{
						return new JsonResult(new CustomResponse<string>() { StatusCode = (int)HttpStatusCodes.BadRequest, Result = false, Message = HttpStatusCodesMessages.BadRequest, Data = string.Format(SystemMessages.msgDataExists, "User") });
					}
					else
					{
		...
		public IActionResult UsersSP(int displayLength, int displayStart, int sortCol, string? search, string sortDir = "asc")
		{
			try
			{
				...
				return new JsonResult(new CustomResponse<string>() { StatusCode = (int)HttpStatusCodes.NotFound, Result = false, Message = HttpStatusCodesMessages.NotFound, Data = string.Format(SystemMessages.msgNotFound, "User") });

			}
			catch (Exception ex)
			{
				_logger.LogError(SystemMessages.msgUnableToFindAppropriateUsers);
				return new JsonResult(new CustomResponse<Exception>() { StatusCode = (int)HttpStatusCodes.InternalServerError, Result = false, Message = HttpStatusCodesMessages.InternalServerError, Data = ex });
			}
		}
		...
		/// <param name="sortCol"> name of column which we want to sort</param>
		/// <param name="search"> word that we want to search in user table </param>
		/// <param name="roleId"> 'asc' or 'desc' direction for sort </param>
		/// <param name="enabled"> 'asc' or 'desc' direction for sort </param>
